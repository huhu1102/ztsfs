...
public interface ContractDao extends JpaRepository<Contract,Long> {
    ...
    @Query(value = "SELECT\n" +
            ...
            "\tand if(:startDateStart != '',DATE_FORMAT( o.createDate, '%Y-%m-%d %k:%i:%s' ) >=:startDateStart, 1 = 1)\n" +
            "\tand if(:startDateEnd!= '', DATE_FORMAT( o.createDate, '%Y-%m-%d %k:%i:%s' ) <=:startDateEnd, 1 = 1)\n" +
            "\tand if(:endDateStart != '',DATE_FORMAT( o.createDate, '%Y-%m-%d %k:%i:%s' ) >=:endDateStart, 1 = 1)\n" +
            "\tand if(:endDateEnd != '', DATE_FORMAT( o.createDate, '%Y-%m-%d %k:%i:%s' ) <=:endDateEnd, 1 = 1)\n" +
